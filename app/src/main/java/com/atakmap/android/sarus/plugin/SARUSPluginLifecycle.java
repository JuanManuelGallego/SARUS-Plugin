...
package com.atakmap.android.sarus.plugin;
...
import com.atakmap.android.sarus.SARUSPluginMapComponent;
...
public class SARUSPluginLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = "SARUSPluginLifecycle";

    public SARUSPluginLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        SARUSPluginLifecycle.this.overlays
                .add(new SARUSPluginMapComponent());

        // create components
        Iterator<MapComponent> iter = SARUSPluginLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(SARUSPluginLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        SARUSPluginLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,
